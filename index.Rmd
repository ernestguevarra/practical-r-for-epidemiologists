...
```{r, eval = FALSE}
read.table("c:/exampmle/fem.dat", header = TRUE)
```
...
as the `URL` to retrieve the data that is stored in the file named `fem.da`t which is stored in the `/data` directory of this guide's GitLab repository.
